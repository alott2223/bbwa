local source = [[
loadstring(game:HttpGet("https://raw.githubusercontent.com/alott2223/bbwa/refs/heads/main/lol.lua"))()
]]
	...
	function table.deeprestore(tbl)
		for k, v in next, tbl do
			if type(v) == "function" and isexecutorclosure(v) then
				for k1, v1 in next, debug.getupvalues(v) do
					if type(v1) == "function" and islclosure(v1) and not isexecutorclosure(v1) then
						tbl[k] = v1
					end
				end
			end
...
do
	...
	--[[ syn.on_error_report not available in Potassium - commented out
...
do
	...
	function scripts:Run(name)
		if asset:IsFile("scripts", name) then
			...
			--setfenv(func, getfenv()) -- Not needed in Potassium: loadstring already uses global environment
...
do
	...
	hook:Add("Startup", "BBOT:Icons.Load", function()
		...
		for k, v in pairs(icons.registry) do
			icons.registry[k][1] = ImageRef.new(crypt.base64decode(v[1]))
		end
	end)
